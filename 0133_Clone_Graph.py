...
class Solution:
    ...
    def cloneGraph(self, node: Optional['Node']) -> Optional['Node']:
        ...
        while stack:
            ...
            # print("res", res)
            # yes
            stack.sort(key=lambda x: x.val) # Could instead: map -> list where len = len map -> loop over map and put ress in order
            #print("stack")
            #for s in stack:
            #    print("     ", s.val)
        #print(res)
        ...
        for i, r in enumerate(res, 1):
            ...
            for sub_node_val in r:
                if sub_node_val in mapping.keys():
                    ...
                    new_node = Node(sub_node_val) # could add curr_node, will add in the next loop iteration
